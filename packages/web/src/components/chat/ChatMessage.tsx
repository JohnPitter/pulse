  ...
  return (
    <div className={cn("flex gap-3", isUser ? "flex-row-reverse" : "flex-row")}>
      <div className={cn(
        ...
        isUser ? "bg-brand" : "bg-neutral-bg3"
      )}>
        {isUser
      ...
      <div className={cn(
        ...
        isUser
          ? "bg-brand text-white text-[13px]"
          : "bg-white border border-stroke text-neutral-fg1"
      )}>
        {isUser
          ? <p className="text-[13px] whitespace-pre-wrap text-white">{content}</p>
        ...
        {createdAt && (
          <p className={`text-[10px] mt-1 ${isUser ? "text-white/70" : "text-neutral-fg3"}`}>
